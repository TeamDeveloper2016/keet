...
(function(window) {
  ...
  Janal.Control.Functions= Class.extend({
    ...
    init: function(root) {
      ...
      if(typeof(root)!== 'undefined')
        this.root   = root;
      $control.load(0, ['/resources/janal/core/jquery.shortcut.core.0.3.9.js','/resources/janal/core/jquery.janal.sticky.min-1.0.0.js','/resources/janal/js/jquery.janal.menu-2.0.1.js','/resources/janal/core/jquery.longclick-1.0.0.js', '/resources/janal/core/jquery.validate.min-1.15.0.js', '/resources/janal/core/jquery.meio.mask.min-1.1.15.js', '/resources/janal/core/jquery.janal.fns-1.6.8.js']);
    ...
    isYear: function(s) {
     ...
     requerido: function(validator, element, value) {
       var ok= false;
         ...
         switch(element.nodeName.toLowerCase()) {
           case 'select':
             ...
             if(options=== '-1' || value=== '-1')
               ...
               ok= options.length> 0 && (element.type=== 'select-multiple' || ($.browser.msie && !(options[0].attributes['value'].specified)? options[0].text : options[0].value).length> 0);
             break;
           case 'input':
             if (validator.checkable(element))
               ok= validator.getLength(value, element)> 0;
             else
               ok= $(element).hasClass('ignore') || !$control.empty(value);
             break;
           default:
             ok= $(element).hasClass('ignore') || !$control.empty(value);
       } // switch       
  ...
  Janal.Control.Validations= Janal.Control.Functions.extend({
    ...
    names         : {
      validations : ['libre', 'max-caracteres', 'min-caracteres', 'igual-caracteres', 'mayor', 'mayor-igual', 'max-valor', 'menor', 'menor-igual', 'min-valor', 'requerido', 'entero', 'entero-signo', 'valor-simple', 'telefono', 'contiene-a', 'igual', 'igual-a', 'menor-a', 'mayor-a', 'diferente-a', 'asterisco', 'moneda', 'moneda-decimal', 'sat', 'flotante', 'flotante-signo', 'mayusculas', 'minusculas', 'vocales', 'rango', 'secuencia-palabra', 'longitud', 'letras', 'alfanumerico', 'texto', 'curp', 'rfc', 'moral', 'texto-especial', 'boleano', 'fecha', 'fecha-menor', 'fecha-mayor', 'registro', 'registro-menor', 'registro-mayor', 'hora', 'hora-completa', 'hora-mayor', 'hora-menor', 'comodin', 'no-permitir', 'ipv4', 'ipv6', 'no-aplica', 'esta-en', 'correo', 'acceso', 'porcentaje', 'depende'],
      ...
      watermarks  : ['entero', 'entero-signo', 'valor-simple', 'decimal', 'decimal-signo', 'sat', 'flotante', 'flotante-signo', 'moneda', 'moneda-decimal', 'mayor', 'mayor-igual', 'mayor-a', 'diferente-a', 'max-valor', 'menor', 'menor-a', 'menor-igual', 'min-valor'],
      formats     : ['libre', 'cambiar-mayusculas', 'especial-mayusculas', 'cambiar-minusculas', 'rellenar-caracter', 'precio', 'cantidad', 'consecutivo', 'descuentos', 'sat', 'siguiente'],
      customs     : []
    },
