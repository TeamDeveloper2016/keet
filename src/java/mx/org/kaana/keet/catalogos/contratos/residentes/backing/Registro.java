...
import mx.org.kaana.keet.catalogos.contratos.residentes.reglas.Transaccion;
...
@Named(value = "keetCatalogosContratosResidentesRegistro")
@ViewScoped
  ...
  public void doLoad() {    		
	...
	private List<SelectionItem> toListSelectionIten(List<ContratistaLote> entities) {
		...
		for(ContratistaLote item: entities)
			regresar.add(new SelectionItem(String.valueOf(item.getIdKey()), "[".concat(item.getDepartamento()).concat("] [").concat(item.getPuesto()).concat("] ").concat(item.getNombres()).concat(" ").concat(item.getPaterno()).concat(" ").concat(item.getMaterno()), item.getIdActivo(), item.getIdNomina(), item.getIdSeguro(), item.getNss(), item.getTipo()));
		return regresar;
	} // toListSelectionIten	
	...
	public void onTransfer(TransferEvent event) {
		...
		try {
			...
			if(transaccion.ejecutar(accion))
				...
				JsfBase.addMessage("Registro de residentes de obra en el desarrollo.", "Ocurrió un error al ".concat(accion.equals(EAccion.PROCESAR) ? "asignar" : "desasignar").concat(" los residentes de obra."), ETipoMensaje.ERROR);			
		} // try
		catch (Exception e) {			
			JsfBase.addMessageError(e);
			Error.mensaje(e);			
