  ...
  public void doLoad() {    		
	...
	private List<SelectionItem> toListSelectionIten(List<ContratoPersonal> entities) {
		...
		for(ContratoPersonal item: entities)
			regresar.add(new SelectionItem(String.valueOf(item.getIdEmpresaPersona()), item.getDescripcion(), item.getIdActivo(), item.getIdNomina(), item.getIdSeguro(), item.getNss()));
		return regresar;
	} // toListSelectionIten
