...
@Named(value = "keetCatalogosContratosResidentesAsignacion")
@ViewScoped
public class Asignacion extends IBaseAttribute implements Serializable {
  ...
  protected void init() {				
    try {						
			...
			this.loadCatalogos();
			Catalogos.toLoadResidentes(this.attrs);
			this.doLoad();
    } // try 
    catch (Exception e) {
      Error.mensaje(e);
  ...
  public void doLoad() {    						
    ...
    try {
      ...
      nombres= (List<UISelectEntity>) UIEntity.seleccione("VistaContratosDto", "residentesSubAsociados", params, columns, Constantes.SQL_TODOS_REGISTROS, "departamento");
