package mx.org.kaana.keet.catalogos.contratos.comun;
...
public abstract class IBaseImportacion extends IBaseImportar implements Serializable{
	...
	protected RegistroContrato registroContrato;

	public RegistroContrato getRegistroContrato() {
		return registroContrato;
	}

	public void setRegistroContrato(RegistroContrato registroContrato) {
		this.registroContrato = registroContrato;
	}
	
	protected void initBase(EArchivosContratos tipoArchivo){
		try {
      ...
      this.attrs.put("retorno", JsfBase.getFlashAttribute("retorno")== null ? "filtro" : JsfBase.getFlashAttribute("retorno"));
			...
			this.registroContrato= new RegistroContrato(Long.valueOf(this.attrs.get("idContrato").toString()));
			loadCombos();
		} // try
		catch (Exception e) {			
			throw e;
	...
	private void loadCombos(){
		...
		List<UISelectEntity> proyectos   = null;
      ...
      especialidad= UIBackingUtilities.toFirstKeySelectEntity(especialidades);		
			...
			doActualizaPlanos();			
			params.clear();
			idCliente= ((TcKeetProyectosDto)DaoFactory.getInstance().findById(TcKeetProyectosDto.class, this.registroContrato.getContrato().getIdProyecto())).getIdCliente();
      ...
      this.attrs.put("prototipos", prototipos);	
			cliente= (TcManticClientesDto) DaoFactory.getInstance().findById(TcManticClientesDto.class, idCliente);
	...
	public void doLoad() {
		...
		EArchivosContratos tipoArchivo= null;
		try {
			tipoArchivo= (EArchivosContratos) this.attrs.get("tipoArchivo");
    ...
    File result            = null;		
		Long fileSize          = 0L;		
		Long idArchivo         = 0L;		
		EArchivosContratos tipo= null;
		try {			
			tipo= (EArchivosContratos) this.attrs.get("tipoArchivo");
      ...
      /*UPLOAD*/
  		...
  		this.attrs.put("file", this.getFile().getName());		
			switch(tipo){ 
				case DOCUMENTOS:
					this.registroContrato.getDocumentos().add((Documento) toArchivo(idArchivo));
					break;
				case GENERADORES:
					this.registroContrato.getGeneradores().add((Generador) toArchivo(idArchivo));
					break;
				case PRESUPUESTOS:
					this.registroContrato.getPresupuestos().add((Presupuesto) toArchivo(idArchivo));
					break;
			} // switch			
		} // try
		catch (Exception e) {
	...
	public String doAceptar() {
		...
		try {
			transaccion= new Transaccion(this.registroContrato, (EArchivosContratos) this.attrs.get("tipoArchivo"));
