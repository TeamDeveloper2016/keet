...
@Named(value = "keetControlesPuntos")
@ViewScoped
  ...
  public void doLoad() {
    ...
    try {      			
      ...
      columns.add(new Columna("factor", EFormatoDinamicos.NUMERO_SIN_DECIMALES));                        
	    this.lazyModel= new FormatLazyModel("VistaCapturaDestajosDto", "puntosRevisionControl", params, columns);		
			this.attrs.put("totalRegistros", DaoFactory.getInstance().toEntitySet("VistaCapturaDestajosDto", "puntosRevisionControl", params).size());
			UIBackingUtilities.resetDataTable();
			columns.add(new Columna("registro", EFormatoDinamicos.FECHA_HORA_CORTA));                  
	    this.lazyModelRechazos= new FormatLazyModel("VistaCapturaDestajosDto", "puntosRevisionRechazosControl", params, columns);			
	...
	private Map<String, Object> toPrepare() {
		...
		try {
			...
			regresar.put("idControl", ((Entity)this.attrs.get("concepto")).getKey());
		} // try
		catch (Exception e) {			
			throw e;
		} // catch		
	...
	private Revision toLoadRevision() {
		...
		try {
			...
			idFigura= seleccionado.toLong("idContratoLoteResidente");
			regresar.setIdFigura(idFigura);
			regresar.setTipo(figura.toLong("tipo"));
			regresar.setIdControl(((Entity) this.attrs.get("concepto")).getKey());
