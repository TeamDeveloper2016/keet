...
@Named(value = "keetControlesExtra")
@ViewScoped
public class Extra extends IBaseAttribute implements Serializable {	

	private static final long serialVersionUID  = 4077399316243366481L;
  ...
  public void doLoad() {
    ...
    try {      			
			...
			puntosGrupos= UIEntity.seleccione("VistaRubrosDto", "byDepartamentoExtraControles", params, Collections.EMPTY_LIST, Constantes.SQL_TODOS_REGISTROS, "codigo");
			this.attrs.put("conceptos", puntosGrupos);									
			estaciones= UIEntity.seleccione("TcKeetControlesDto", "byClaveNivel", params, Collections.EMPTY_LIST, Constantes.SQL_TODOS_REGISTROS, "codigo");
	...
	private ConceptoExtra toLoadConceptoExtra() {
		...
		try {
			...
			regresar.setIdFigura(seleccionado.toLong("idContratoLoteResidente"));
			regresar.setTipo(figura.toLong("tipo"));
			regresar.setIdControl(Long.valueOf(this.attrs.get("especialidad").toString()));
