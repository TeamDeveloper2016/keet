package mx.org.kaana.keet.estaciones.masivos.beans;

import java.io.Serializable;
import java.time.LocalDate;
import java.time.LocalDateTime;
import mx.org.kaana.keet.db.dto.TcKeetControlesDto;

/**
 *@company KAANA
 *@project KAJOOL (Control system polls)
 *@date 5/05/2020
 *@time 11:38:25 PM 
 *@author Team Developer 2016 <team.developer@kaana.org.mx>
 */

public class Control extends TcKeetControlesDto implements Cloneable, Serializable {

	private static final long serialVersionUID=-6170305461966609460L;

	public Control() {
		this("", "", "", 0L, 1L, 1L, -1L);
	}
	public Control(String clave, String codigo, String nombre, Long nivel, Long ultimo, Long idEmpaqueUnidadMedida, Long idUsuario) {
		super();
		this.setIdControl(-1L);
		this.setIdPlantilla(2L);
		this.setNivel(nivel);
		this.setClave(clave);
		this.setUltimo(ultimo);
		this.setCodigo(codigo);
		this.setNombre(nombre);
		this.setDescripcion(nombre);
		this.setIdEmpaqueUnidadMedida(idEmpaqueUnidadMedida);
		this.setCantidad(1D);
		this.setIdControlEstatus(1L);
		this.setInicio(LocalDate.now());
		this.setTermino(LocalDate.now());
		this.setIdUsuario(idUsuario);
		this.setRegistro(LocalDateTime.now());
		this.setCosto(0D);
		this.setAbono1(0D);
		this.setAbono2(0D);
		this.setAbono3(0D);
		this.setAbono4(0D);
		this.setAbono5(0D);
		this.setAbono6(0D);
		this.setAbono7(0D);
		this.setAbono8(0D);
		this.setAbono9(0D);
		this.setAbono10(0D);
		this.setAbono11(0D);
		this.setAbono12(0D);
		this.setAbono13(0D);
		this.setAbono14(0D);
		this.setAbono15(0D);
		this.setAbono16(0D);
		this.setAbono17(0D);
		this.setAbono18(0D);
		this.setAbono19(0D);
		this.setAbono20(0D);
		this.setAbono21(0D);
		this.setAbono22(0D);
		this.setAbono23(0D);
		this.setAbono24(0D);
		this.setAbono25(0D);
		this.setAbono26(0D);
		this.setAbono27(0D);
		this.setAbono28(0D);
		this.setAbono29(0D);
		this.setAbono30(0D);
		this.setAbono31(0D);
		this.setAbono32(0D);
		this.setAbono33(0D);
		this.setAbono34(0D);
		this.setAbono35(0D);
		this.setAbono36(0D);
		this.setAbono37(0D);
		this.setAbono38(0D);
		this.setAbono39(0D);
		this.setAbono40(0D);
		this.setAbono41(0D);
		this.setAbono42(0D);
		this.setAbono43(0D);
		this.setAbono44(0D);
		this.setAbono45(0D);
		this.setAbono46(0D);
		this.setAbono47(0D);
		this.setAbono48(0D);
		this.setAbono49(0D);
		this.setAbono50(0D);
		this.setAbono51(0D);
		this.setAbono52(0D);
		this.setAbono53(0D);
		this.setAbono54(0D);
		this.setAbono55(0D);
		this.setCargo1(0D);
		this.setCargo2(0D);
		this.setCargo3(0D);
		this.setCargo4(0D);
		this.setCargo5(0D);
		this.setCargo6(0D);
		this.setCargo7(0D);
		this.setCargo8(0D);
		this.setCargo9(0D);
		this.setCargo10(0D);
		this.setCargo11(0D);
		this.setCargo12(0D);
		this.setCargo13(0D);
		this.setCargo14(0D);
		this.setCargo15(0D);
		this.setCargo16(0D);
		this.setCargo17(0D);
		this.setCargo18(0D);
		this.setCargo19(0D);
		this.setCargo20(0D);
		this.setCargo21(0D);
		this.setCargo22(0D);
		this.setCargo23(0D);
		this.setCargo24(0D);
		this.setCargo25(0D);
		this.setCargo26(0D);
		this.setCargo27(0D);
		this.setCargo28(0D);
		this.setCargo29(0D);
		this.setCargo30(0D);
		this.setCargo31(0D);
		this.setCargo32(0D);
		this.setCargo33(0D);
		this.setCargo34(0D);
		this.setCargo35(0D);
		this.setCargo36(0D);
		this.setCargo37(0D);
		this.setCargo38(0D);
		this.setCargo39(0D);
		this.setCargo40(0D);
		this.setCargo41(0D);
		this.setCargo42(0D);
		this.setCargo43(0D);
		this.setCargo44(0D);
		this.setCargo45(0D);
		this.setCargo46(0D);
		this.setCargo47(0D);
		this.setCargo48(0D);
		this.setCargo49(0D);
		this.setCargo50(0D);
		this.setCargo51(0D);
		this.setCargo52(0D);
		this.setCargo53(0D);
		this.setCargo54(0D);		
	}
	
	@Override
	public Class toHbmClass() {
		return TcKeetControlesDto.class;
	}
	
	@Override
	public Control clone() throws CloneNotSupportedException {
		Control clone= new Control();
		clone.setIdControl(-1L);
		clone.setIdPlantilla(this.getIdPlantilla());
		clone.setNivel(this.getNivel());
		clone.setClave(this.getClave());
		clone.setUltimo(this.getUltimo());
		clone.setCodigo(this.getCodigo());
		clone.setNombre(this.getNombre());
		clone.setDescripcion(this.getDescripcion());
		clone.setIdEmpaqueUnidadMedida(this.getIdEmpaqueUnidadMedida());
		clone.setInicio(this.getInicio());
		clone.setTermino(this.getTermino());
		clone.setCantidad(this.getCantidad());
		clone.setIdControlEstatus(this.getIdControlEstatus());
		clone.setIdUsuario(this.getIdUsuario());
		clone.setRegistro(LocalDateTime.now());
		clone.setCosto(0D);
		clone.setAbono1(0D);
		clone.setAbono2(0D);
		clone.setAbono3(0D);
		clone.setAbono4(0D);
		clone.setAbono5(0D);
		clone.setAbono6(0D);
		clone.setAbono7(0D);
		clone.setAbono8(0D);
		clone.setAbono9(0D);
		clone.setAbono10(0D);
		clone.setAbono11(0D);
		clone.setAbono12(0D);
		clone.setAbono13(0D);
		clone.setAbono14(0D);
		clone.setAbono15(0D);
		clone.setAbono16(0D);
		clone.setAbono17(0D);
		clone.setAbono18(0D);
		clone.setAbono19(0D);
		clone.setAbono20(0D);
		clone.setAbono21(0D);
		clone.setAbono22(0D);
		clone.setAbono23(0D);
		clone.setAbono24(0D);
		clone.setAbono25(0D);
		clone.setAbono26(0D);
		clone.setAbono27(0D);
		clone.setAbono28(0D);
		clone.setAbono29(0D);
		clone.setAbono30(0D);
		clone.setAbono31(0D);
		clone.setAbono32(0D);
		clone.setAbono33(0D);
		clone.setAbono34(0D);
		clone.setAbono35(0D);
		clone.setAbono36(0D);
		clone.setAbono37(0D);
		clone.setAbono38(0D);
		clone.setAbono39(0D);
		clone.setAbono40(0D);
		clone.setAbono41(0D);
		clone.setAbono42(0D);
		clone.setAbono43(0D);
		clone.setAbono44(0D);
		clone.setAbono45(0D);
		clone.setAbono46(0D);
		clone.setAbono47(0D);
		clone.setAbono48(0D);
		clone.setAbono49(0D);
		clone.setAbono50(0D);
		clone.setAbono51(0D);
		clone.setAbono52(0D);
		clone.setAbono53(0D);
		clone.setAbono54(0D);
		clone.setAbono55(0D);
		clone.setCargo1(0D);
		clone.setCargo2(0D);
		clone.setCargo3(0D);
		clone.setCargo4(0D);
		clone.setCargo5(0D);
		clone.setCargo6(0D);
		clone.setCargo7(0D);
		clone.setCargo8(0D);
		clone.setCargo9(0D);
		clone.setCargo10(0D);
		clone.setCargo11(0D);
		clone.setCargo12(0D);
		clone.setCargo13(0D);
		clone.setCargo14(0D);
		clone.setCargo15(0D);
		clone.setCargo16(0D);
		clone.setCargo17(0D);
		clone.setCargo18(0D);
		clone.setCargo19(0D);
		clone.setCargo20(0D);
		clone.setCargo21(0D);
		clone.setCargo22(0D);
		clone.setCargo23(0D);
		clone.setCargo24(0D);
		clone.setCargo25(0D);
		clone.setCargo26(0D);
		clone.setCargo27(0D);
		clone.setCargo28(0D);
		clone.setCargo29(0D);
		clone.setCargo30(0D);
		clone.setCargo31(0D);
		clone.setCargo32(0D);
		clone.setCargo33(0D);
		clone.setCargo34(0D);
		clone.setCargo35(0D);
		clone.setCargo36(0D);
		clone.setCargo37(0D);
		clone.setCargo38(0D);
		clone.setCargo39(0D);
		clone.setCargo40(0D);
		clone.setCargo41(0D);
		clone.setCargo42(0D);
		clone.setCargo43(0D);
		clone.setCargo44(0D);
		clone.setCargo45(0D);
		clone.setCargo46(0D);
		clone.setCargo47(0D);
		clone.setCargo48(0D);
		clone.setCargo49(0D);
		clone.setCargo50(0D);
		clone.setCargo51(0D);
		clone.setCargo52(0D);
		clone.setCargo53(0D);
		clone.setCargo54(0D);
		return clone;
	}
	
}
