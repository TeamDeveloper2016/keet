...
import mx.org.kaana.libs.reportes.Security;
...
public class Exportar implements Serializable {
  ...
  public Exportar(String fileName, String source, Map<String, Object> params, JRDataSource jRDataSource) {
    ...
    this(fileName, source, formato, params, jRDataSource, pagination, false);
  }
  
	public Exportar(String fileName, String source, EFormatos formato, Map<String, Object> params, JRDataSource jRDataSource , Boolean pagination, Boolean pdfConSeguridad) {
  ...
  private void pdf() throws Exception {
    ...
    try {
      ...
      if (this.pdfConSeguridad!=null && this.pdfConSeguridad) {
        security= new Security(this.fileName.concat(".pdf"));
        LOG.debug("PDF-Colocando seguridad en el archivo: ".concat(this.fileName).concat(".pdf"));
		    security.execute();
      } // if
    } // try
    catch (Exception e) {
  ...
  private void xls() throws Exception {
    ...
    try {
      ...
      configuration.setOnePagePerSheet(JsfBase.getAutentifica()!= null ? false: this.pagination);      
      exporter.setConfiguration(configuration);			      
		  exporter.exportReport();
