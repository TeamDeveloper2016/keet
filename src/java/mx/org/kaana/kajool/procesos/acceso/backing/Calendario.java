...
public class Calendario extends Comun implements Serializable {
  ...
  public void doSelect(SelectEvent selectEvent) {
    ...
    if(Objects.equals(item, null)) {
      ...
      this.attrs.put("hoy", Fecha.formatear(Fecha.FECHA_NOMBRE_DIA, (LocalDateTime)calendario.getInitialDate()));
      this.toLoadCuentasPagar(Fecha.formatear(Fecha.FECHA_ESTANDAR, (LocalDateTime)calendario.getInitialDate()));
    } // if
    else {
  ...
  public void doDate(SelectEvent selectEvent) {
    LocalDateTime fecha= (java.time.LocalDateTime)selectEvent.getObject();
    this.attrs.put("hoy", Fecha.formatear(Fecha.FECHA_NOMBRE_DIA, fecha.plusDays(1)));
    this.toLoadCuentasPagar(Fecha.formatear(Fecha.FECHA_ESTANDAR, fecha.plusDays(1)));
  }
  ...
  public void doView(String date) {
    ...
    if(date.indexOf(" ")== 3) {
      ...
      month= Fecha.getNumeroMesCorto(name);
      ...
      if(date.indexOf(" ")> 3) {
        name = date.substring(0, date.indexOf(" "));
        month= Fecha.getNumeroMesLargo(name);
        if(date.indexOf(",")> 0)
