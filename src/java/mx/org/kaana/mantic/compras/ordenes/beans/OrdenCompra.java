...
public class OrdenCompra extends TcManticOrdenesComprasDto implements Serializable {
  ...
  private Long itEmpresa;
	...
	public OrdenCompra(Long key) {
		this(-1L, 0D, -1L, -1L, "0.00", Math.abs(new Random().nextLong())* -1L, "0.00", new Long(Calendar.getInstance().get(Calendar.YEAR)), Calendar.getInstance().get(Calendar.YEAR)+ "00000", 1L, 0D, 1L, LocalDate.now(), 1L, -1L, 0D, 0D, 1D, 2L, "", -1L, 1L, 0D);
	}
