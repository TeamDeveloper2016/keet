...
public class MotorBusqueda implements Serializable {
	...
	public Importado toArticuloImagen(Long idImagen) throws Exception {
        ...
        /*UPLOAD*/
				if(imagen!= null)
					regresar= new Importado(imagen.getNombre(), imagen.getArchivo(), EFormatos.FREE, imagen.getTamanio(), imagen.getTamanio(), BYTES, imagen.getRuta(), "", -1L);			
			} // if
			else
    ...
    try {      
      ...
      params.put("idArticulo", this.idArticulo);      
        ...
        for (ArticuloImagen item: items) {
          item.setImportado(new Importado(item.getArchivo(), EFormatos.JPG.getContent(), EFormatos.JPG, item.getTamanio(), item.getTamanio(), BYTES, item.getRuta(), item.getOriginal(), item.getArchivo(), item.getIdArchivo()));
