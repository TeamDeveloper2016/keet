  ...
  public void doEliminar() {
    ...
    try {
      ...
      columns.add(new Columna("limiteCredito", EFormatoDinamicos.MONEDA_SAT_DECIMALES));      
			columns.remove(0);
			columns.remove(1);
      this.attrs.put("estatusFiltro", (List<UISelectEntity>) UIEntity.seleccione("TcManticCotizacionesEstatusDto", "row", params, columns, "nombre"));
  ...
  public void doReporteFacturas(String nombre) throws Exception {
	...
	public void doSendmail() {
      ...
      params.put("url", Configuracion.getInstance().getPropiedadServidor("sistema.dns"));
			...
			files.add(new Attachment("logo", ECorreos.COTIZACIONES.getImages().concat(Configuracion.getInstance().getEmpresa("logo")), Boolean.TRUE));
